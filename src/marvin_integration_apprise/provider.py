...
and a safe ``http`` client via ``ctx``, and it returns results. It never touches the database or the
event bus — the core owns persistence and dispatch.

Apprise (github.com/caronc/apprise) fans one message out to 100+ services — Slack, Discord, Telegram,
email, and more — through a single URL scheme. Here the Apprise URL(s) are the credential (stored
behind ``secret_ref``), and a ``notify`` action sends a title/body. Wire it to events with an
integration event subscription so "on event X → notify" replaces the old core notifier.
"""
